...
export async function competitionRoutes(fastify: FastifyInstance) {
  fastify.post('/', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    // Broadcast: competition created (draft)
    await publishAnnouncement({
      type: 'competition.created',
      title: competition.title,
      body: `New competition created: ${competition.title}`,
      data: {
  ...
  fastify.post('/:id/vote', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const updatedEntry = await prisma.entry.update({
      ...
      data: {
        likes: { increment: 1 },
        // Score: views*1 + likes*10 + comments*5 (SPEC-compliant)
        score: { increment: 10 },
      },
    });
  ...
  fastify.post('/:id/start', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    await publishAnnouncement({
      ...
      title: competition.title,
      body: `Competition started: ${competition.title}`,
      data: {
        competitionId: competition.id,
        status: competition.status,
        type: competition.type,
        shareUrl: `${process.env.ARENA_URL || 'http://localhost:3001'}/game/${competition.id}`,
      },
    });
  ...
  fastify.post('/:id/publish', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const shareUrl = `${process.env.ARENA_URL || 'http://localhost:3001'}/game/${id}`;
    await publishAnnouncement({
      type: 'competition.published',
      title: updated.title,
      body: `Competition published: ${updated.title}`,
      data: {
  ...
  fastify.post('/:id/end', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    await publishAnnouncement({
      ...
      title: competition.title,
      body: `Competition ended: ${competition.title}`,
      data: {
        competitionId: competition.id,
        status: competition.status,
        type: competition.type,
